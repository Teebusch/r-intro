...
Now we can use the `read_csv()` function to load the data straight from the web into an R data frame in our computer's working memory.
...
When we used the `skim()` function, you might have noticed the double colon `::` between the package name and
the function name. This is a way to call a function without first loading the package
