...
<kbd>Ctrl</kbd>/<kbd>Cmd</kbd> + <kbd>↑</kbd> to search through previous commands.
...
format that allows you to mix code and the output it generates with plain text. This
allows you to keep your notes and your code together and is great for writing reproducible
documents.[^easy-start-1]

[^easy-start-1]: This entire book was written with *{bookdown},* which also uses
    RMarkdown.

::: {.exercise}
Go to `File ▸ New File ▸ R Notebook` to create a new R notebook and save it in your
project's working directory as `analysis.Rmd`. Note that the file has appeared in
RStudio's `Files` tab, which shows you all the files in your project folder.
:::

The new notebook already contains some example content. You can see that the R code is
written inside special blocks, called *code chunks*. You can create new code chunks with
the green button on the top right of the script panel or with the keyboard shortcut
<kbd>Ctrl</kbd>/<kbd>Cmd</kbd> + <kbd>Option</kbd>/<kbd>Alt</kbd> + <kbd>I</kbd>

You can run an entire code chunk by clicking the green arrow on the right of the chunk.
You can also run individual by using <kbd>Ctrl</kbd>/<kbd>Cmd</kbd> + <kbd>Enter</kbd> or
the buttons on the top right of the script panel.
